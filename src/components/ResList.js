...
const ResList = () => {
    const { restaurants, filteredRestaurants, setFilteredRestaurants, bestRestaurants, topResChains } = useRestaurants()
    ...
    return useInfo ? (restaurants?.length === 0 ? <Shimmer /> : (
        <div className="w-full mt-8 flex flex-col justify-start items-center flex-wrap container">
            ...
            <div className="w-[80%] mt-12 mb-6">
                <h1 className="text-left font-bold text-2xl">{topResChains[2]?.card?.card?.title}</h1>
            </div>
            ...
            <BestRestaurants bestRestaurants={bestRestaurants[0]?.card?.card?.brands} title={bestRestaurants[0]?.card?.card?.title} />
            <BestRestaurants bestRestaurants={bestRestaurants[1]?.card?.card?.brands} title={bestRestaurants[1]?.card?.card?.title} />
            <BestRestaurants bestRestaurants={bestRestaurants[2]?.card?.card?.brands} title={bestRestaurants[2]?.card?.card?.title} />
        </div>
